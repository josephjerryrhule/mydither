// scripts/dither-ink-006.mjs
// Pixel-perfect extraction for Day 006 / Haven.
// Splits the drawing into:
// - sprout.png (the emerging sprout)
// - dot.png (the hovering dewdrop/sun)
// - ground.png (the soil line at bottom)
import sharp from 'sharp';
import { mkdirSync, writeFileSync } from 'node:fs';

const SRC = 'assets/006-haven-source.jpg';
const OUT_DIR = 'public/006';
const TARGET_W = 1080;
...
// Classifiers
function isDot(x, y) { return x <= 450 && y <= 380; }
function isGround(x, y) { return y > 790; }
function isSprout(x, y) { return !isDot(x, y) && !isGround(x, y); }
...
const sproutMeta = await saveSprite('sprout', isSprout);
const dotMeta = await saveSprite('dot', isDot);
