...
// the flower head is drawn line-by-line using staggered vector strokes.
// The signature sun dot materializes behind the blossom.
...
export const BEATS = {
  ...
  flower: { start: 90, end: 220 },     // 3.0–7.3s — flower head is drawn
...
// Flower head drawing progress (staggered sepal & petal strokes)
export const flowerLeftProgress = (f: number) =>
  easeOutCubic(norm(f, 90, 170));

export const flowerRightProgress = (f: number) =>
  easeOutCubic(norm(f, 110, 190));

export const flowerCenterProgress = (f: number) =>
  easeOutCubic(norm(f, 130, 220));
